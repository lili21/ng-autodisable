  ...
  .directive('ngAutodisable', [ '$parse', function($parse) {
    ...
    function linkFn(scope, element, attrs) {
      ...
      if( attrs.hasOwnProperty(CLICK_ATTR) ) {
          handler = handlerInstance(element[0],
                                    CLICK_EVENT,
                                    getLoadingClass(attrs),
                                    getCallbacks(attrs[CLICK_ATTR]));
      } else  if ( attrs.hasOwnProperty(SUBMIT_ATTR) ) {
          handler = handlerInstance(element[0].querySelector('button[type=submit]'),
                                    SUBMIT_EVENT,
                                    getLoadingClass(attrs),
    ...
    function handlerInstance(elementToDisable, eventName, loadingClass, callbacks) {
      ...
      function disableElement() {
        // elementToDisable.setAttribute(DISABLED, true);
        elementToDisable[DISABLED] = true;
        if (loadingClass) {
          elementToDisable.classList.add(loadingClass);
        }
      }
      ...
      function enableElement() {
        // elementToDisable.setAttribute(DISABLED, false);
        elementToDisable[DISABLED] = false;
        if (loadingClass) {
          elementToDisable.classList.remove(loadingClass);
        }
      }
